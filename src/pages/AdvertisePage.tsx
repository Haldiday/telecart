  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [settingsResult, cardsResult, sectionsResult] = await Promise.all([
          supabase.from('advertise_page_settings').select('*').limit(1).maybeSingle(),
          supabase.from('advertise_cards').select('*').order('sort_order'),
          supabase.from('advertise_sections').select('*').order('sort_order'),
        ]);
        ...
        <>
          ...
          <main className="flex-1">
            ...
            {settings?.hero_visible && (
              <section 
                className="pt-[100px] md:pt-[140px] lg:pt-[90px] pb-[80px] px-4 md:px-8 bg-gradient-to-br from-gray-50 to-white"
                style={{
                  opacity: 0,
            ...
            {settings && (settings.dynamic_sections_heading_part1 || settings.dynamic_sections_heading_part2) && (
              <section 
                className="py-8 pt-2 -mt-24 px-4 md:px-8 bg-white"
                style={{
                  opacity: 0,
                  animation: 'fadeUp 0.6s ease-out forwards',
                  animationDelay: '0.7s'
            ...
            {sections.filter(section => section.is_visible).map((section, index) => {
              ...
              return (
                <section 
                  key={section.id} 
                  className="py-10 px-4 md:px-6 bg-gray-50"
                  style={{
                    opacity: 0,
                    animation: 'fadeUp 0.6s ease-out forwards',
