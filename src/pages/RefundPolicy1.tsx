...
export default function RefundPolicy1() {
    ...
    const loadPage = async () => {
      try {
        const { data, error } = await supabase
          .from('legal_pages')
          .select('*')
          .eq('slug', 'refund-policy-1')
          .maybeSingle();

        if (error) throw error;
        setPage(data as LegalPage);
      } catch (error) {
        console.error('Error loading refund policy 1:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 bg-white pt-24 md:pt-36 pb-12">
        <div className="container mx-auto px-4 md:px-8 max-w-[1250px]">
          <h1 className="text-3xl md:text-[32px] font-semibold mb-8 text-[#222222]">{page?.title || 'Refund Policy 1'}</h1>
          <RichTextContent
