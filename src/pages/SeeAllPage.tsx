...
export default function SeeAllPage() {
  ...
  useEffect(() => {
    ...
    const loadSections = async () => {
      ...
      if (data && mounted) {
        // Filter only the sections we want: cards, ads_3col, offers
        const filteredSections = data.filter(
          (section) => ['cards', 'ads_3col', 'offers'].includes(section.section_type) && section.is_visible
        );
        ...
        <div className="py-4">
          {sections.length > 0 ? (
            sections.map((section, index) => {
              ...
              const sectionContent = <Component sectionId={section.id} backgroundColor={section.background_color} />;
