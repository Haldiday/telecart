...
export default function Ads3ColSeeAllPage() {
  ...
  useEffect(() => {
    ...
    const loadSections = async () => {
      ...
      if (data && mounted) {
        // Filter only ads_3col sections
        const filteredSections = data.filter(
          (section) => section.section_type === 'ads_3col' && section.is_visible
        );
        ...
        <div className="py-4">
          {sections.length > 0 ? (
            sections.map((section, index) => {
              const sectionContent = <Ads3ColSection sectionId={section.id} backgroundColor={section.background_color} />;
