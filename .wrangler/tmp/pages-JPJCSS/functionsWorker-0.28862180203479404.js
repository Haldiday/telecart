...
var init_config = __esm({
  "helpers/config.js"() {
    init_functionsRoutes_0_8425275591599936();
  ...
  "../node_modules/@supabase/realtime-js/dist/module/lib/serializer.js"() {
    ...
    Serializer = class {
      ...
      _encodeJsonUserBroadcastPush(message2) {
        ...
        const encodedUserPayload = encoder4.encode(JSON.stringify(userPayload)).buffer;
...
var otpMap, OTP_TTL_MS;
var init_otpStore = __esm({
  "helpers/otpStore.js"() {
    init_functionsRoutes_0_8425275591599936();
    otpMap = /* @__PURE__ */ new Map();
    OTP_TTL_MS = 5 * 60 * 1e3;
    __name(storeOtp, "storeOtp");
...
var init_msg91 = __esm({
  "helpers/msg91.js"() {
    ...
    MSG91Service = class {
      ...
      getFromEmail() {
        return this.config.msg91FromEmail || (this.config.msg91EmailDomain ? `no-reply@${this.config.msg91EmailDomain}` : "no-reply@mailer91.com");
      }
...
var init_utils = __esm({
  "helpers/utils.js"() {
    ...
    __name(validateOTP, "validateOTP");
    __name(getBearerToken, "getBearerToken");
  }
});

// helpers/supabaseAuth.js
async function getSupabaseUserByToken(env, token) {
...
async function isAdminUser(env, userId) {
  try {
    const { data, error } = await getSupabaseAdmin(env).from("user_roles").select("role").eq("user_id", userId).eq("role", "admin").maybeSingle();
    return !!data;
  } catch {
    return false;
  }
...
var init_supabaseAuth = __esm({
  "helpers/supabaseAuth.js"() {
    ...
    init_utils();
    init_supabase();
    __name(getSupabaseUserByToken, "getSupabaseUserByToken");
    __name(isAdminUser, "isAdminUser");
    __name(requireAdminAuth, "requireAdminAuth");
  }
});

// api/user/change-email/request.js
async function onRequestPost({ request, env }) {
  try {
    ...
    const { data: existingUser } = await supabase.from("users").select("id").eq("email", new_email).single();
    if (existingUser) {
    ...
    const message2 = error instanceof Error ? error.message : "Failed to send OTP";
    return jsonResponse({ success: false, message: message2 }, 500);
  }
}
...
async function signJwt(payload, secret, expiresIn) {
  ...
  return new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setExpirationTime(expiresInSeconds).sign(encoder2.encode(secret));
}
...
// api/user/change-email/verify.js
async function onRequestPost2({ request, env }) {
  try {
    ...
    const { data: updatedUser, error } = await supabase.from("users").update({ email: new_email }).eq("id", user.id).select("*").single();
    ...
    const message2 = error instanceof Error ? error.message : "Invalid or expired OTP";
    return jsonResponse({ success: false, message: message2 }, 400);
  }
}
...
// api/auth/check-email-account.js
...
var init_check_email_account = __esm({
  "api/auth/check-email-account.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_supabase();
    init_utils();
...
function generateWhatsAppOtp() {
  const values = new Uint32Array(1);
  crypto.getRandomValues(values);
  return String(1e5 + values[0] % 9e5);
}
async function sendWhatsAppOtp(phone, otp, env) {
  ...
  try {
    ...
    const body = { receiver, values: { 1: otp } };
    ...
    const validResponse = responseBody && responseBody.messaging_product === "whatsapp" && Array.isArray(responseBody.contacts) && responseBody.contacts.length > 0 && Array.isArray(responseBody.messages) && responseBody.messages[0] && responseBody.messages[0].message_status === "accepted";
    if (!validResponse) {
...
var DEFAULT_WHATSAPP_API_URL, REQUEST_TIMEOUT_MS, WhatsAppServiceError;
var init_whatsapp = __esm({
  "helpers/whatsapp.js"() {
    init_functionsRoutes_0_8425275591599936();
    DEFAULT_WHATSAPP_API_URL = "https://adminapis.backendprod.com/lms_campaign/api/whatsapp/template/54p9v2sbfn/process";
    REQUEST_TIMEOUT_MS = 1e4;
    WhatsAppServiceError = class extends Error {
      static {
        __name(this, "WhatsAppServiceError");
      }
      constructor(message2, reason, status = 502) {
        super(message2);
    ...
    __name(generateWhatsAppOtp, "generateWhatsAppOtp");
    __name(sendWhatsAppOtp, "sendWhatsAppOtp");
  }
});

// api/auth/check-whatsapp-account.js
...
var init_check_whatsapp_account = __esm({
  "api/auth/check-whatsapp-account.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_supabase();
    init_utils();
...
function isDisposableDomain(domain) {
  return DISPOSABLE_DOMAINS.has(domain) || /(?:^|\.)(mailinator|tempmail|yopmail|trashmail|maildrop|guerrilla|mailnesia|tmpmail|mintemail|mailsac|mailosaur)(?:\.|$)/.test(domain);
}
function getBooleanValue(value) {
  return typeof value === "object" && value !== null ? value.value === true : value === true;
}
var CACHE_TTL_MS, REQUEST_TIMEOUT_MS2, ABSTRACT_API_URL, DISPOSABLE_DOMAINS, cache, EmailValidationServiceError, EmailValidationService;
var init_emailValidation = __esm({
  "helpers/emailValidation.js"() {
    init_functionsRoutes_0_8425275591599936();
    ...
    __name(isDisposableDomain, "isDisposableDomain");
    __name(getBooleanValue, "getBooleanValue");
    EmailValidationServiceError = class extends Error {
      static {
        __name(this, "EmailValidationServiceError");
      }
    ...
    EmailValidationService = class {
      static {
        __name(this, "EmailValidationService");
      }
      async validateEmail(email, env) {
        ...
        try {
          ...
          const isDisposable = getBooleanValue(emailQuality.is_disposable) || getBooleanValue(payload.is_disposable_email) || getBooleanValue(payload.is_disposable) || getBooleanValue(payload.disposable);
          const result = {
...
function consumeRateLimit(key, maxRequests, windowMs = 15 * 60 * 1e3) {
  const now = Date.now();
...
var init_send_email_otp = __esm({
  "api/auth/send-email-otp.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_config();
    init_msg91();
    init_emailValidation();
    init_utils();
    init_rateLimiter();
    emailValidationService = new EmailValidationService();
    __name(getEmailLogDetails, "getEmailLogDetails");
    __name(getEnvironmentStatus, "getEnvironmentStatus");
    __name(onRequestPost6, "onRequestPost");
  }
});

// helpers/whatsappOtpStore.js
function storeWhatsAppOtp(phone, otp) {
  otpMap2.set(phone, { otp, expiresAt: Date.now() + OTP_TTL_MS2, attempts: 0 });
  console.info("[WhatsApp OTP] Stored OTP", { phoneSuffix: phone.slice(-4), expiresAt: Date.now() + OTP_TTL_MS2 });
}
function deleteWhatsAppOtp(phone) {
  otpMap2.delete(phone);
}
function verifyWhatsAppOtp(phone, otp) {
  const entry = otpMap2.get(phone);
...
var OTP_TTL_MS2, MAX_ATTEMPTS, otpMap2, WhatsAppOtpError;
var init_whatsappOtpStore = __esm({
  "helpers/whatsappOtpStore.js"() {
    init_functionsRoutes_0_8425275591599936();
    OTP_TTL_MS2 = 5 * 60 * 1e3;
    MAX_ATTEMPTS = 5;
    otpMap2 = /* @__PURE__ */ new Map();
    WhatsAppOtpError = class extends Error {
      static {
        __name(this, "WhatsAppOtpError");
      }
      constructor(message2, reason) {
        super(message2);
    ...
    __name(storeWhatsAppOtp, "storeWhatsAppOtp");
    __name(deleteWhatsAppOtp, "deleteWhatsAppOtp");
    __name(verifyWhatsAppOtp, "verifyWhatsAppOtp");
  }
});

// api/auth/send-whatsapp-otp.js
...
var init_send_whatsapp_otp = __esm({
  "api/auth/send-whatsapp-otp.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_utils();
    init_rateLimiter();
    init_whatsappOtpStore();
    init_whatsapp();
...
// helpers/signup.js
function parseSignupData(value) {
...
var init_signup = __esm({
  "helpers/signup.js"() {
    init_functionsRoutes_0_8425275591599936();
    __name(parseSignupData, "parseSignupData");
  }
});
...
async function onRequestPost8({ request, env }) {
  try {
    ...
    if (!existingUser) {
      const { data: newUser, error: insertError } = await supabase.from("users").insert({ email, is_verified: true, ...signupData }).select("*").single();
      if (insertError) throw insertError;
      user = newUser;
    } else if (!existingUser.is_verified) {
      const { data: updatedUser, error: updateError } = await supabase.from("users").update({ is_verified: true }).eq("id", existingUser.id).select("*").single();
...
var init_verify_email_otp = __esm({
  "api/auth/verify-email-otp.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_config();
    init_supabase();
...
// api/auth/verify-whatsapp-otp.js
async function onRequestPost9({ request, env }) {
  ...
  try {
    ...
    let user = existingUser;
    if (!user) {
      const { data, error } = await supabase.from("users").insert({ phone, phone_verified: true, is_verified: true, ...signupData }).select("*").single();
      if (error) throw error;
      user = data;
...
var init_verify_whatsapp_otp = __esm({
  "api/auth/verify-whatsapp-otp.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_config();
    init_supabase();
...
async function executeQuery(spec, env) {
  const supabase = getSupabaseAdmin(env);
  try {
    ...
    const message2 = err instanceof Error ? err.message : "Query execution failed";
    return { data: null, error: { message: message2 } };
  }
}
var init_dbExecutor = __esm({
  "helpers/dbExecutor.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_supabase();
    __name(applyFilters, "applyFilters");
    __name(applyOrders, "applyOrders");
    __name(applyLimitAndSingle, "applyLimitAndSingle");
...
async function onRequestPost10({ request, env }) {
  try {
    ...
    const adminActions = /* @__PURE__ */ new Set(["insert", "update", "delete", "upsert"]);
...
// helpers/zohoTokens.js
async function createZohoPrefillToken({ env, userId, ttlMs = 3e5 }) {
  const supabase = getSupabaseAdmin(env);
  const token = crypto.randomUUID();
  const expiresAt2 = Date.now() + Math.max(1e3, ttlMs);
...
async function validateZohoPrefillToken({ env, token }) {
  if (!token) return null;
  const supabase = getSupabaseAdmin(env);
  const { data, error } = await supabase.from("zoho_prefill_tokens").select("token, user_id, expires_at").eq("token", token).maybeSingle();
  if (error) throw error;
  if (!data) return null;
...
async function markZohoPrefillTokenUsed({ env, token }) {
  const supabase = getSupabaseAdmin(env);
  const { error } = await supabase.from("zoho_prefill_tokens").delete().eq("token", token);
  if (error) throw error;
}
var init_zohoTokens = __esm({
  "helpers/zohoTokens.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_supabase();
    __name(createZohoPrefillToken, "createZohoPrefillToken");
    __name(validateZohoPrefillToken, "validateZohoPrefillToken");
    __name(markZohoPrefillTokenUsed, "markZohoPrefillTokenUsed");
  }
});

// api/forms/generate-token.js
async function onRequestPost11({ request, env }) {
  try {
    ...
    const requestedTtl = typeof rawBody.ttlMs === "number" ? Math.round(rawBody.ttlMs) : void 0;
    const ttlMs = typeof requestedTtl === "number" ? Math.min(MAX_TTL_MS, Math.max(MIN_TTL_MS, requestedTtl)) : 5 * 6e4;
...
var init_generate_token = __esm({
  "api/forms/generate-token.js"() {
    ...
    __name(verifyTokenPayload, "verifyTokenPayload");
  }
});

// api/forms/prefill.js
async function handlePrefill(token, env) {
  ...
  try {
    ...
    const supabase = getSupabaseAdmin(env);
    const { data: user, error: userError } = await supabase.from("users").select("email, full_name, company_name, phone").eq("id", tokenRecord.userId).single();
    if (userError || !user) {
...
var init_prefill = __esm({
  "api/forms/prefill.js"() {
    init_functionsRoutes_0_8425275591599936();
    init_supabase();
    init_zohoTokens();
    init_utils();
    __name(handlePrefill, "handlePrefill");
...
async function getSignatureKey(secretAccessKey, dateStamp, region, service) {
  const kDate = await hmac(encoder3.encode("AWS4" + secretAccessKey), encoder3.encode(dateStamp));
  const kRegion = await hmac(kDate, encoder3.encode(region));
  const kService = await hmac(kRegion, encoder3.encode(service));
  return await hmac(kService, encoder3.encode("aws4_request"));
}
function encodeR2Path(path) {
  return path.split("/").map((part) => encodeURIComponent(part).replace(/%2F/g, "/")).join("/");
}
...
async function signR2Request({ method, url, headers, payloadHash, accessKeyId, secretAccessKey, region = "auto", service = "s3" }) {
  ...
  const canonicalHeaders = Object.keys(headers).map((name) => `${name.toLowerCase()}:${headers[name].trim()}`).sort().join("\n") + "\n";
  ...
  const stringToSign = [
    ...
    amzDate,
    credentialScope,
    await sha2562(encoder3.encode(canonicalRequest))
  ].join("\n");
  const signingKey = await getSignatureKey(secretAccessKey, dateStamp, region, service);
  const signature = toHex2(await hmac(signingKey, encoder3.encode(stringToSign)));
...
async function deleteFromR2(env, bucket, path) {
  ...
  const encodedKey = encodeR2Path(objectKey);
  const url = `${getR2Endpoint(config.accountId)}/${bucket}/${encodedKey}`;
  const payloadHash = await sha2562(encoder3.encode(""));
  const headers = {
    host: `${config.accountId}.r2.cloudflarestorage.com`,
    "x-amz-content-sha256": payloadHash
  };
...
var init_r2 = __esm({
  "helpers/r2.js"() {
    ...
    __name(encodeR2Path, "encodeR2Path");
    __name(normalizePublicUrl, "normalizePublicUrl");
    __name(getR2PublicUrl, "getR2PublicUrl");
    __name(getR2Config, "getR2Config");
    __name(getR2Endpoint, "getR2Endpoint");
    __name(signR2Request, "signR2Request");
...
async function onRequestGet3({ _request: _request2, env }) {
  const config = getR2Config(env);
...
async function onRequestPost13({ request, env }) {
  ...
  const body = await request.json();
  const bucket = typeof body.bucket === "string" ? body.bucket : void 0;
...
async function onRequestPut({ request, env }) {
  try {
    ...
    if (full_name !== void 0 && (typeof full_name !== "string" || full_name.length < 2 || full_name.length > 60)) {
      return jsonResponse({ success: false, message: "Full name must be between 2 and 60 characters" }, 400);
    }
    if (company_name !== void 0 && typeof company_name !== "string") {
      return jsonResponse({ success: false, message: "Company name must be a string" }, 400);
    }
    if (company_name !== void 0 && company_name.length > 120) {
      return jsonResponse({ success: false, message: "Company name must be less than or equal to 120 characters" }, 400);
    }
    const supabase = getSupabaseAdmin(env);
    const { data: updatedUser, error } = await supabase.from("users").update({ full_name, company_name, profile_photo }).eq("id", payload.id).select("*").single();
    if (error) throw error;
...
// ../.wrangler/tmp/pages-JPJCSS/functionsRoutes-0.8425275591599936.mjs
var routes;
var init_functionsRoutes_0_8425275591599936 = __esm({
  "../.wrangler/tmp/pages-JPJCSS/functionsRoutes-0.8425275591599936.mjs"() {
    init_request();
    init_verify5();
    init_check_email_account();
    init_check_whatsapp_account();
    init_logout();
    init_me();
    init_send_email_otp();
    init_send_whatsapp_otp();
    init_verify_email_otp();
    init_verify_whatsapp_otp();
    init_query();
    init_generate_token();
    init_prefill();
    init_prefill();
