...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'vendor_guidelines_settings' AND policyname = 'Public read vendor guidelines settings') THEN
        CREATE POLICY "Public read vendor guidelines settings"
            ON public.vendor_guidelines_settings
            FOR SELECT
            USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'vendor_guidelines_settings' AND policyname = 'Admins can write vendor guidelines settings') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_vendor_guidelines_settings_updated_at') THEN
