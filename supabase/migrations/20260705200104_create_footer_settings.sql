...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'footer_settings' AND policyname = 'Public read footer settings') THEN
        CREATE POLICY "Public read footer settings" ON public.footer_settings
            FOR SELECT USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'footer_settings' AND policyname = 'Admin write footer settings') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_footer_settings_updated_at') THEN
